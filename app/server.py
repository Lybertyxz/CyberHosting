...
def partials(file):
    ...
    if request.cookies.get("mb_session_id"):
        user = jwt.decode(
            request.cookies.get("mb_session_id"), JWT_SECRET, algorithms=["HS256"]
        )
    ...
    if file == "profile.html" or file == "transfer.html":
        ...
        if backupdir:
            err, out = subprocess.getstatusoutput(
                "zip -r backup.zip {}".format(backupdir)
            )
    ...
    if request.form.get("email"):
        if request.form.get("password"):
            if request.form.get("last_name") and request.form.get("first_name"):
                ...
                return redirect(
                    "/?view=register&message=Database Error - Please try again later"
                )
    return
    ...
    if request.form.get("sum"):
        if request.form.get("recipient"):
            if request.form.get("pin"):
                user = None
                user = jwt.decode(
                    request.cookies.get("mb_session_id"),
                    JWT_SECRET,
                    algorithms=["HS256"],
                )
                ...
                if out1 is None and out2 is None:
                    ...
                    resp = make_response(
                        redirect("/?view=profile&message=Transfer successfully done")
                    )
                    resp.set_cookie("mb_session_id", token)
                    return resp
